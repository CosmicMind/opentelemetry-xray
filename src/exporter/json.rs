...
fn validate_time_range(start: &SystemTime, end: &SystemTime) -> Result<(), ExportError> {
    if start > end {
...
/// Generates a new OpenTelemetry `TraceId`
pub fn generate_trace_id() -> TraceId {
    let mut random_bytes = [0u8; 16]; // SpanId must be exactly 8 bytes
    let _ = OsRng.try_fill_bytes(&mut random_bytes);
    TraceId::from_bytes(random_bytes)
}

/// Generates a new OpenTelemetry `SpanId`
pub fn generate_span_id() -> SpanId {
    let mut random_bytes = [0u8; 8]; // SpanId must be exactly 8 bytes
    let _ = OsRng.try_fill_bytes(&mut random_bytes);
...
impl JsonExporter {
    ...
    fn format_xray_trace_id(trace_id: TraceId) -> String {
        ...
        format!("1-{}-{}", &hex[..8], &hex[8..])
    }

    /// Create an X‑Ray JSON segment from a span.
    pub fn create_segment(&self, span_data: SpanData) -> Result<Value, ExportError> {
        // Validate and process timestamps.
        validate_time_range(&span_data.start_time, &span_data.end_time)
        ...
        let parent_id = if span_data.parent_span_id != SpanId::INVALID {
            Some(span_data.parent_span_id.to_string())
        } else {
            None
        };
        ...
        for kv in span_data.attributes.iter() {
            ...
            if key.starts_with("http.request.") {
                validate_value(&value, ValueType::HttpRequest, &trace_id)?;
                request.insert(
                    key.strip_prefix("http.request.").unwrap().to_string(),
                    json!(value),
                );
            } else if key.starts_with("http.response.") {
                validate_value(&value, ValueType::HttpResponse, &trace_id)?;
                response.insert(
                    key.strip_prefix("http.response.").unwrap().to_string(),
                    json!(value),
                );
            } else if key.starts_with("annotation.") {
                validate_value(&value, ValueType::Annotation, &trace_id)?;
                annotations.insert(
                    key.strip_prefix("annotation.").unwrap().to_string(),
                    json!(value),
                );
            } else if key.starts_with("metadata.") {
                validate_value(&value, ValueType::Metadata, &trace_id)?;
                metadata.insert(
                    key.strip_prefix("metadata.").unwrap().to_string(),
                    json!(value),
                );
        ...
        let span_type = if parent_id.is_some() {
            "subsegment"
        } else {
            "segment"
        };
...

        {
            ...
            if let Some(parent) = parent_id {
                segment_obj.insert("parent_id".to_string(), json!(parent));
            }
    ...
    pub fn export_batch(&mut self, batch: Vec<SpanData>) -> Result<(), ExportError> {
        ...
        for span_data in batch {
            ...
            for event in span_data.events {
                ...
                let event_span = SpanData {
                    span_context: SpanContext::new(
                        span_data.span_context.trace_id(),
                        generate_span_id(),
                        span_data.span_context.trace_flags(),
                        false,
                        span_data.span_context.trace_state().clone(),
                    ),
                    parent_span_id: span_data.span_context.span_id(),
                    span_kind: span_data.span_kind.clone(),
                    ...
                    status: Status::Ok,
                    instrumentation_scope: span_data.instrumentation_scope.clone(),
                };
